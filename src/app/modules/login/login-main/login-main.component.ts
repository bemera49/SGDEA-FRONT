...
import {
  IConfigInicial,
  IConfigParams,
} from "../../../interfaces/configuracion-inicial.interface";
...
export class LoginMainComponent implements OnInit, OnDestroy {
    ...
    this.authService
      ...
      .subscribe((responseApi: IConfigInicial) => {
        this.globalAppService
          .resolveResponse(responseApi)
          .then((responseGlobal) => {
            if (responseGlobal == true) {
              if (
                responseApi.dataStatus &&
                Number(responseApi.valorCsInicial) ===
                  environment.statusTodoNumber.activo
              ) {
                this.configParams();
              }
            }
    ...
    this.authService
      ...
      .subscribe((responseApi: IConfigParams) => {
        this.globalAppService
          ...
          .then((responseGlobal) => {
            if (responseGlobal == true) {
              const dataCaptchaSiteKey = responseApi.data.find(
                (item) => item.llaveCsParams === data.csParam,
              );
  ...
  detectedCloseModalLicensingMessage() {
    this.subscriptionChildrenService$ =
      this.changeChildrenService.closeComponent.subscribe(
  ...
  aceptaLicencia() {
    this.validateShowPanelLicensing().then((response) => {
      if (response === 1) {
        ...
        this.openModalLicensingMessage(
          LoginLicensingMessageComponent,
          "75%",
          true,
        );
      }
  ...
  login() {
    ...
    this.authService
      .authPost(
        `${environment.versionApiDefault}site/login`,
      ...
      .subscribe(
        (data) => {
          ...
          this.globalAppService
            .resolveResponse(this.responseLogin)
            .then((res) => {
              ...
              if (responseResolverRespuesta == true) {
                ...
                localStorage.setItem(
                  ...
                  this.authService.encryptAES(
                    this.responseLogin.dataMenu,
                    false,
                  ),
                );
                localStorage.setItem(
                  environment.hashTimeOut,
                  this.responseLogin.data.tiempoInactividadCgGeneral,
                );
                ...
                if (
                  this.responseLogin.status == environment.statusErrorValidacion
                ) {
  ...
  analyzeLocalStorage() {
    ...
    let hashMailSgdea = localStorage.getItem(environment.hashMailSgdea);
    let hashMailSgdeaDecipted = this.authService.decryptAES(hashMailSgdea);
    if (hashMailSgdeaDecipted != null) {
      /** Comparar el usuario actual con el usuario que realizo el proceso de radicacion email en la sesion anterior */
      if (
        hashMailSgdeaDecipted["userLoginApp"] !=
        this.loginForm.controls["username"].value
      ) {
        localStorage.removeItem(environment.hashMailSgdea);
        localStorage.removeItem(environment.hashMailInitialListSgdea);
      }
    } else {
      localStorage.removeItem(environment.hashMailSgdea);
      localStorage.removeItem(environment.hashMailInitialListSgdea);
  ...
  validateShowPanelLicensing() {
    return new Promise((resolve) => {
      this.authService
        .authPost(
          `${environment.versionApiDefault}site/user-status-licensing`,
          this.loginForm.value,
          false,
        )
  ...
  ngOnDestroy() {
    ...
    if (!!this.subscriptionChildrenService$)
      this.subscriptionChildrenService$.unsubscribe();
  }
}

...
@Component({
  selector: 'app-settings-app-providers-update',
  templateUrl: './settings-app-providers-update.component.html',
  styleUrls: ['./settings-app-providers-update.component.css']
})
export class SettingsAppProvidersUpdateComponent implements OnInit {
  ...
  textForm = 'Actualizar proveedor'; // i18n
  // Ruta a redirigir
  redirectionPath = '/setting/providers-index';
  /** BreadcrumbOn  */
  breadcrumbOn = [
    { 'name': 'Configuración', 'route': '/setting' },
    { 'name': 'Proveedores de envío', 'route': this.redirectionPath }
  ];
  ...
  submitFormReceive(data) {
    ...
    this.restService.restPut( this.versionApi + 'configuracionApp/cg-proveedores/update', data, this.authorization)
      .subscribe((res) => {

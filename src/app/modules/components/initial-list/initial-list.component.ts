...
export class InitialListComponent implements AfterViewInit, OnInit, AfterViewChecked, OnDestroy, OnChanges {
  ...
  @Input() canSelectAll: boolean = true; // parametro para permitir seleccionar todo
  @Input() canViewItemNumber: boolean = true; // parametro para permitir ver el numero del item
  @Input() viewPagination: boolean = true; // parametro para permitir ver la paginacion
  ...
  @Input() showButtonRequest: boolean = false; // Muestra el botones tipo de solicitudes
  ...
  @Input() showStatusLabel: boolean = true;
  /*** Fin Configuraciones para datatables ***/

  /** Clases de los contenedores iniciales */
  @Input() classMainConten: string = "main-content mt-0";
  ...
  listActosAdministrativos: any;
  /** Variables para consultas de servicios */

  @Input() NoDecrypt: boolean = false;
  ...
  @Output() public buttonRequestEmitter = new EventEmitter<any>();
  ...
  subscriptionDataChangeShippingElectronic: any;
  ...
  @Input() menu: string = ""; // Menu de la aplicación
  constructor(
  ...
  configDtOptions() {
    return new Promise((resolve) => {
      ...
      this.dtOptions = {
        paging: this.viewPagination,
        info: this.viewPagination,
        dom: "lfrtBip",
        buttons: this.viewPagination ? buttonsTable : [],
        lengthChange: false,
  ...
  dataTablesInitService(new_form: boolean = false) {
    let params: any = {};
    if (new_form == false) {
      if (this.dataFilter) {
  ...
  selectRow(row: any) {
    ...
    if (row.rowSelect && index === -1) {
      this.selectedRows.push(row);
    } else if (!row.rowSelect && index > -1) {
      this.selectedRows.splice(index, 1);
    }
  ...
  clickButtonRequest(event) {
    this.buttonRequestEmitter.emit(event);
  }

...
export class UsersUsersIndexComponent implements OnInit {
  ...
  routeLoadDataTablesService: string =
    environment.versionApiDefault + "user/index";
  dtTitles: any = [
  ...
  menssageStatus: string =
    "El usuario tiene radicados asignados y no se puede inactivar";
  ...
  /** Datos para el proceso correspondiente a sgda express*/
  sgdaVal = environment.sgda;
  resServicessgdaVal: any;
  resServicessgdaValErr: any;
  ...
  routeChangeStatus: string =
    environment.versionApiDefault + "user/change-status";
  /**
  ...
  getTokenLS() {
    ...
    this.lhs.getToken().then((res: string) => {
      ...
      this.paramsOk = this.encryptService.decryptAES(
        this.params,
        this.authorization,
      );
  ...
  menuReceiveData(event) {
    switch (event.action) {
      case "add":
        // Si la cantidad de registros mostrados es igual a la cantidad de los enviroments y el sgda expres
        if (this.sgdaVal.ocultarModulos == true) {
  ...
  /** Mendiante esta opción se llama al servicio que muestra el mensaje de comercial para adquirir sgda completo */
  messageExpress() {
    /** Cantidad de registros a mostrar en el listado incial, limitante que se envia al bakend */
    let params = {
      data: this.sgdaVal.limitarCreacionUsuarios,
    };

    this.restService
      .restPost(this.versionApi + "user/comercial", params, this.authorization)
      .subscribe(
        (res) => {
          this.resServicessgdaVal = res;

          if (this.resServicessgdaVal.status == 200) {
            this.router.navigate(["/" + this.initBotonCreateRoute]);
          }

          this.globalAppService
            .resolveResponse(this.resServicessgdaVal, false)
            .then((res) => {
              const responseResolveResponse = res;
            });
        },
        (err) => {
          this.resServicessgdaValErr = err;
          // Evaluar respuesta de error del servicio
          this.globalAppService
            .resolveResponseError(this.resServicessgdaValErr)
            .then((res) => {});
        },
      );
  }

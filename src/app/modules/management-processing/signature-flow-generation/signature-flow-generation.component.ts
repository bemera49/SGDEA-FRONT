...
formFecha = this.fb.group({
  ...
  public goBack() {
    this.modal.openAction('Confirmación', '¿Está seguro que desea cancelar el flujo de firma en curso?', () => {  
      this.router.navigate([`/pqrs/pqrs-view/${this.activatedRoute.snapshot.params['id']}`])
      })
  }
...
public revisordataConfig = {
  ...
  crearFlujo() {
    ...
    if(!this.areAllSubarraysEmpty(data)) {
     ...
     return this.rest$.post(`api/flujo/radicado/guardar/${this.currentID}/${userData.idDataCliente}/${userData.dependencia.idGdTrdDependencia}`, body)
      .pipe(
        catchError((err => {
    ...
    if(this.form.valid) {
      const { tipoFirma, rol, tipoUsuario, usuario } = this.form.getRawValue();
        ...
        this.registros[index].push([{
            "nombreUsuario": 
            (usuario as any).user_detalle.nombreUserDetalles + ' ' + (usuario as any).user_detalle.apellidoUserDetalles,
  ...
  if(this.isStepReady() ) {
    this.modal.openAction('¿Está seguro de la accion a realizar?', '', () => {
      this.crearFlujo().subscribe(response => {
        this.modal.openNotify('Exito', 'Flujo de firma creado exitosamente' ,true).afterClosed().subscribe(closed => this.router.navigate(['/signin']))
      })
    })
    return;
  } 

...
import { DocManagUploadTrdErrorsComponent } from '../../document-management/doc-manag-upload-trd-errors/doc-manag-upload-trd-errors.component';
            ...
            case HttpEventType.Response: // Cuando termina la carga
                if (event.body) {
                  ...
                  this.encryptService.decryptAES(this.responseService.encrypted, this.authorization).then((res) => {
                    ...
                    console.log(responseResolveResponse == true)
                    if (responseResolveResponse == true) {
                      ...
                      this.uploadValid = false;
                      console.log(event.body.decrypted)
                      if (Array.isArray(event.body.decrypted.data.error) && event.body.decrypted.data.error[0]) {
                        Swal({
                          title: "Algo está mal..",
                          position: "top",
                          toast: true,
                          ...
                          this.openDialogUploadError(event.body.decrypted.data.error);
                        });
                      } else if (event.body.decrypted.data.error) {
                        Swal({
                          title: "Algo está mal..",
                          position: "top",
                          toast: true,
                          ...
                          type:'error',
                          text:event.body.decrypted.data.error,
                          width: 800
                        })
                      } else {
                           Swal({
                             ...
                             showConfirmButton: false,
                             type:'success',
                             text: `Se han cargado ${event.body.decrypted.register} registros. ${event.body.decrypted.message}`,
                             width: 600,
                           })
                        }
                    }

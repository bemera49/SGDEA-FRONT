...
export class FilingIndexComponent implements OnInit, OnDestroy {
  ...
  routeLoadDataTablesService: string =
    this.versionApi + "radicacion/radicados/index";
  // Configuración para el proceso change status
  routeChangeStatus: string =
    this.versionApi + "radicacion/radicados/change-status";
  ...
  public menuReceiveData(event) {
    ...
    switch (event.action) {
              ...
              if (result.value) {
                ...
                if (this.isActo)
                  this.router.navigate([
                    "/" + this.initBotonCreateRoute + "/actoAdmin",
                  ]);
                else this.router.navigate(["/" + this.initBotonCreateRoute]);
              }
            });
          });
        } else {
      ...
      case "includeInFile":
        ...
        break;
      case "shippingReady":
        this.floatingButtonService.changeTransactionShippingReady(
          this.eventClickButtonSelectedData,
        );
        break;
      ...
      case "generateStickerFiling":
        this.transactionGenerateStickerFiling(
          this.eventClickButtonSelectedData,
        );
        break;
  ...
  public transactionGenerateStickerFiling(event): void {
    if (event.length > 0) {
      ...
      this.restService
        .restPost(
          this.versionApi + "radicacion/radicados/generate-sticker-filing",
          params,
          this.authorization,
        )
        .subscribe(
          (responseService) => {
            ...
            this.globalAppService
              .resolveResponse(response, false)
              .then(async (res) => {
                const responseResolveResponse = res;
                if (responseResolveResponse == true) {
                  if (
                    response.hasOwnProperty("dataFile") &&
                    response["dataFile"] !== ""
                  ) {
          ...
          (err) => {
            ...
            // Evaluar respuesta de error del servicio
            this.globalAppService
              .resolveResponseError(this.resErrServices)
              .then((res) => {});
          },
        );
    }
  ...
  public transactionGenerateNumberFiling(event): void {
    if (event.length > 0) {
      ...
      this.restService
        .restPost(
          this.versionApi + "radicacion/radicados/generate-number-filing",
          params,
          this.authorization,
        )
        .subscribe(
  ...
  public transactionPrintExternalLabel(event): void {
    if (event.length > 0) {
      ...
      this.restService
        .restPost(
          this.versionApi + "radicacion/transacciones/print-external-sticker",
          params,
          this.authorization,
        )
        .subscribe(
  ...
  operacionesTiposRadicados(event: any) {
    if (event.length > 0) {
      ...
      if (
        params.event[0] != undefined &&
        params.event[0].estado == "Rechazado"
      ) {
        //
      ...
      this.restService
        .restPost(
          this.versionApi + "radicacion/radicados/radi-multi-acciones",
          params,
          this.authorization,
        )
        .subscribe(
    ...
    if (dataObserva.status) {
      ...
      switch (this.operationDialogObserva) {
        case "schedule":
          ...
          break;
        case "send":
          this.floatingButtonService.changeTransactionReasign(
            this.eventClickButtonSelectedData,
            dataObserva.data,
            this.operationDialogObserva,
          );
          break;
        case "asignarRadicado":
          this.floatingButtonService.changeTransactionReasign(
        ...
        case "attachment":
          ...
          break;
        case "returnFiling":
          this.floatingButtonService.changeReturnFiling(
            this.eventClickButtonSelectedData,
            dataObserva.data.observacion,
            true,
          );
          this.statusModalMain = true;
          break;
        case "withdrawal":
          this.floatingButtonService.changeTransactionWithdrawal(
            this.eventClickButtonSelectedData,
            dataObserva.data.observacion,
          );
          break;
        case "voboRequest":
          this.floatingButtonService.changeTransactionVobo(
            this.eventClickButtonSelectedData,
            dataObserva.data,
            this.operationDialogObserva,
          );
          break;
        case "copyInformaded":
          this.floatingButtonService.changeTransactionCopyInfo(
            this.eventClickButtonSelectedData,
            dataObserva.data,
            this.operationDialogObserva,
          );
          break;
        case "finalizeFiling":
          this.floatingButtonService.changeTransactionFinalizeFiling(
            this.eventClickButtonSelectedData,
            dataObserva.data,
            this.operationDialogObserva,
          );
        ...
        case "includeInFile":
          ...
          break;
        case "returnPqrToCitizen":
          this.floatingButtonService.changeReturnPqrToCitizen(
            this.eventClickButtonSelectedData,
            dataObserva.data,
          );
          break;

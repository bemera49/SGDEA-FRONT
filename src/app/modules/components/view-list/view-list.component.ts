...
export class ViewListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  @Output() public enviarCorreoEmiter = new EventEmitter<any>(); // Data a retornar al formulario de expediente
  ...
  menuButtons: any = [
    ...
    { icon: 'cloud_upload', title: 'Editar', action: 'Actualizar', data: '' },
  ];
  ...
  gifsgda: string = environment.frontUrl + 'assets/img/logos/03-42-11-849_512.gif';
  ...
  getCallUrl(authori) {
    ...
    this.restService.restGetParams(this.versionApi + this.reuteLoadView, params, authori).subscribe(
      (res) => {
        ...
        this.loading = false;
        console.log({"responseServiceView": this.responseServiceView.data});
        // Evaluar respuesta del servicio
        this.globalAppService.resolveResponse(this.responseServiceView, this.isredirectionPath, this.redirectionPath).then((res) => {
          ...
          if (responseResolveResponse == true) {
            ...
            this.data1 = data1;
            this.data2 = data2;
            const emailValue = this.data1.find(data => data.alias === 'Email')?.value || this.data1.find(data => data.alias === 'Correo electrónico')?.value;
            this.enviarCorreoEmiter.emit(emailValue);
            //this.enviarCorreoEmiter.emit(this.data1.find(data => data.alias === 'Email')?.value);
            //this.enviarCorreoEmiter.emit(this.data1.find(data => data.alias === 'Correo electrónico')?.value);
            // base64Image: this.data1.find(data => data.alias === 'Firma Radicado');
            ...
            //pintamos la firma radicado
            console.log('Firma', this.responseServiceView.firmaRadicado);
            if(this.responseServiceView.firmaRadicado){
              this.firmaRadicado = this.responseServiceView.firmaRadicado[0]?? '';
            }
            ...
            if (this.dtCollapStatus == true) {
              ...
              if (this.responseServiceView.dataUserTramitador) {
                // Buscando el alias igual a Medio Recepcion
                let medioRecepcionValue = this.responseServiceView.data.find(data => data.alias === 'Medio Recepción')?.value;
                this.responseServiceView.dataUserTramitador.medioRecepcion = medioRecepcionValue;
              ...
              if(this.dataRowCorrespondencia.length > 0){
                this.showCorrespondencia = true
              }
  ...
  docFocusCor(data, id, dataDoc) {
    ...
    this.valselectPri = null;
    this.valselect = null;
    let aux = this.menuButtonsCor
    /*
    if (dataDoc === "4-72") {
      const item = {
        icon: "info",
        title: "Detalle Operador",
        action: "detalle472",
        data: "",
      };
      const index = aux.findIndex((i) => i.title === item.title);
      if (index > -1) {
        aux.splice(index, 1);
      } else {
        aux.push(item);

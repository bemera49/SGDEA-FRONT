...
export class UsersOperationsFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.moduleForm = this.formBuilder.group({
      nombreRolOperacion: new UntypedFormControl(
        "",
        Validators.compose([Validators.required]),
      ),
      aliasRolOperacion: new UntypedFormControl(
        "",
        Validators.compose([Validators.required]),
      ),
      moduloRolOperacion: new UntypedFormControl(
        "",
        Validators.compose([Validators.required]),
      ),
      idRolModuloOperacion: new UntypedFormControl(
        "",
        Validators.compose([Validators.required]),
      ),
      bankFilterCtrl: new UntypedFormControl(
        "",
        Validators.compose([
          // Validators.required
        ]),
  ...
  getListmodules(authori) {
    ...
    this.restService
      .restGet(versionApi + "roles/roles-modulos-operaciones/index", authori)
      .subscribe(
        (res) => {
  ...
  onSearchId(id, authori) {
    ...
    this.restService
      .restGetParams(
        versionApi + "roles/roles-operaciones/index-one",
        params,
        authori,
      )
      .subscribe(
  ...
  filterBanks() {
    ...
    this.filteredBanks.next(
      this.listModuleMenu.filter(
        (listModuleMenu) =>
          listModuleMenu.val.toLowerCase().indexOf(search) > -1,
      ),
    );
  }

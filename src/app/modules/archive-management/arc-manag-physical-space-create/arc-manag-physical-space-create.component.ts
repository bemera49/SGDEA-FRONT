  ...
  submitFormReceive(data) {
    ...
    if ( data['numeroGaPiso'].length < 1 ) {
      this.sweetAlertService.text18nGet().then( (res) => {
        const dataLang = res;
        this.sweetAlertService.sweetInfo('Algo está mal', 'Error al diligenciar, por favor verifique e intente nuevamente');
      });
    } else {
        ...
        .subscribe((res) => {
          ...
          this.globalAppService.resolveResponse(this.resSerFormSubmit, false ).then((res) => {
            ...
            if (responseResolveResponse == true) {
              // Muestra el mensaje
              this.sweetAlertService.showNotification('success', 'La información ha sido guardada con éxito.');

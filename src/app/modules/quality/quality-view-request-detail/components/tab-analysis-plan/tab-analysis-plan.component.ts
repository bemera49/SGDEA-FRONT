...
import { Meeting, Meetings, RequestAnalysis } from './interface/analysis-requests';
...
export class TabAnalysisPlanComponent implements OnInit, OnChanges {
  ...
  private redirection = inject(Router);
  private vi = inject(ValidateInternetService);
  private user = inject(RestService);
  private _downloadService = inject(RequestDetailService);
  ...
  dateCreateId: number
  selectedValuesConfirm: { [key: number]: number } = {};
  ...
  analysisForm = new FormGroup({
    question_i: new FormControl(null, Validators.required),
    question_ii: new FormControl(null, Validators.required),
    ...
    meetings: this.formBuilder.array(
      [this.formBuilder.group({
        ...
        help_file: new FormControl({ value: '', disabled: true }),
      })
      ]),
  ...
  onSelectItem(index: number, value: number) {
    this.selectedValuesConfirm[index] = value;
  }
  ...
  ngOnChanges(): void {
    ...
    if (this.analysis_plan) {
      this.analysisForm.get('question_i').setValue(parseInt(this.analysis_plan.questions[0].respuesta));
      this.analysisForm.get('question_ii').setValue(parseInt(this.analysis_plan.questions[1].respuesta));
      ...
      if (this.analysis_plan && this.analysis_plan.meetings && this.analysis_plan.meetings.length > 0) {
        ...
        console.log(this.analysisForm.get('meetings'))
        this.analysis_plan.meetings.forEach((meeting, index) => {
          ...
          meetingsControl.push(this.formBuilder.group({
            ...
            help_file: ['', meeting.help_file]
          }));
          this.selectedValuesConfirm[index] = meeting.isConfirm;
          this.fileNameInput[index] = meeting.help_file?.name;
          this.fileId[index] = meeting.help_file?.id;

          console.log(this.fileNameInput[index], 'filename');
        });
      ...
      console.log(this.dateCreate, 'lafecha');
  ...
  ngOnInit(): void {
    ...
    console.log(this.analysis_plan)
  ...
  onAddItems(): void {
    if (this.enableFunctions || this.idTipeTask == 4) {
  ...
  parseData(): RequestAnalysis {
    ...
    const newMeeting = meeting.map((item, index) => {
      ...
      return {
        ...item,
        'email': email?.join(','),
        'isConfirm': this.selectedValuesConfirm[index]
      }
    })
  ...
  private disableForm() {
    ...
    Object.keys(this.analysisForm.controls).forEach(key => {
      ...
      if (key == 'observation' && this.idTipeTask == 4) {
        control.enable();
      } else {
        control.disable();
      }
    ...
    const meetingsControl = this.analysisForm.get('meetings') as FormArray;
    meetingsControl.controls.forEach((control: FormGroup, index) => {
      Object.keys(control.controls).forEach(key => {
        if (this.idTipeTask == 4) {
    ...
    if (this.analysisForm.valid && this.meetingLength !== 0) {
      ...
      const dialogRef = this.modal.open(ActionModalComponent, {
        ...
        onConfirmAction: () => {
          ...
          if (this.analysis_plan) {
            newForm = { ...valueForm, id: this.analysis_plan.id, request_id: this.idParam }
            // formData = parseFormDataPlan<RequestAnalysis>(newForm);
          } else {
            newForm = { ...valueForm, id: this.dateCreateId, request_id: this.idParam }
            // formData = parseFormDataPlan<RequestAnalysis>(newForm);

          }
          const formData = parseFormDataPlan<RequestAnalysis>(newForm);
          console.log('newfrm', newForm)
  ...
  updateMeet(isSend: boolean): void {
    if (this.analysisForm.valid && this.meetingLength !== 0) {
      ...
      const dialogRef = this.modal.open(ActionModalComponent, {
        ...
        onConfirmAction: () => {
          ...
          if (this.analysis_plan) {
            newForm = {
              ...valueForm,
              id: this.analysis_plan.id,
              request_id: this.idParam,
              is_send: isSend,
              question_i: this.analysis_plan.questions[0].respuesta,
              question_ii: this.analysis_plan.questions[1].respuesta,
              which: this.analysis_plan.questions[0].cual
            }
          } else {
            newForm = {
              ...valueForm,
              id: this.dateCreateId,
              request_id: this.idParam,
              is_send: isSend,
              question_i: this.analysis_plan.questions[0].respuesta,
              question_ii: this.analysis_plan.questions[1].respuesta,
          ...
          this.analysisRequests.updateMeets(newForm).subscribe({
            next: (response) => {
              ...
              if (!isSend) {
                this.toast.open({
                  title: '139',
                  description: 'Devolución Exitosa.'
                });
              }
              this.dateUpdate = response.data.fecha_hrs_u;
              const observation: DataUserObservation = {
  ...
  updateConfirm(isSave: boolean): void {
    ...
    const dialogRef = this.modal.open(ActionModalComponent, {
      ...
      onConfirmAction: () => {
        ...
        this.analysisRequests.updateConfirm(newForm).subscribe({
          next: (response) => {
  ...
  private onchangemeetings(response: any): void {
    ...
    response.forEach((meeting, index) => {
      meetingsControl.push(this.formBuilder.group({
        id: [meeting.id],
        area: [parseInt(meeting.gdTrdDependencia_id), Validators.required],
        ...
        help_file: ['', meeting.help_file?.name],
      }));
      this.selectedValuesConfirm[index] = meeting.isConfirm;
      this.fileNameInput[index] = meeting.help_file?.name;
      this.fileId[index] = meeting.help_file?.id;
  ...
  onFileSelected(event: Event, index: number): void {
    ...
    this._validateSC.validateCorrupt(archivo).then(res => {
      ...
      if (res.status) {
        this.toast.open({
          title: '035',
          description: 'Por favor verifique el formato del archivo o documento a cargar. El formato permitido es PDF.',
        })
      }
  ...
  downloadFile(idDocument: number, index: number): void {
    if (idDocument) {

      this._downloadService.downloadFileV1(idDocument).subscribe(item => {
        this.saveFile(item, index);
      })
    }
  }

  private saveFile(blob: Blob, index: number): void {
    const downloadLink = document.createElement('a');
    const url = window.URL.createObjectURL(blob);
    downloadLink.href = url;
    downloadLink.download = this.fileNameInput[index];
  ...
  updateTableHeaders() {
    if (this.idTipeTask == 20) {
      this.headerTablaItems.push({ name: 'Confirma', key: '' });
      this.headerTablaItems.push({ name: 'Cancela', key: '' });
    } else {
      this.headerTablaItems = this.headerTablaItems.filter(item => item.name !== 'Confirma' && item.name !== 'Cancela');
    }
  }

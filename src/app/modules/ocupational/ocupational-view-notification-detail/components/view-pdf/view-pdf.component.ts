...
import { Component, OnInit, Input, OnDestroy, ViewChild, ElementRef, inject, SimpleChanges } from '@angular/core';
...
import { ViewPdfService } from "./Services/view-pdf.service";
...
export class ViewPdfFileComponent implements OnDestroy, OnInit {
  ...
  downloadFileService() {
    ...
    if (this.validateIt.validateConnection()) {
        ...
        const id = this.idRequest;

        this.viewServices.getRespuesta(id).subscribe(item => {

          this.responseServiceFile = item;
          ...
          if (this.responseServiceFile.status) {
            ...
            this.openPdf(this.responseServiceFile.data.base64Pdf);
          } else {

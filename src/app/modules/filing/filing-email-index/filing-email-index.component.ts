...
export class FilingEmailIndexComponent implements OnInit, OnDestroy {
  ...
  dtTitles: any = [
    { title: "No. Radicado", data: "" },
    { title: "No. Cuenta contarto", data: "" },
    { title: "Fecha Radicacion", data: "" },
    { title: "Fecha envío", data: "date" },
    { title: "De", data: "NameAndAddress" },
    { title: "Tipo solicitud", data: "" },
  ...
  ngOnInit() {
    ...
    let dataLocalStorage = localStorage.getItem(environment.hashMailSgdea);
  ...
  goFilingCreate() {
    ...
    this.dataHashMailSgdea = this.authService.decryptAES(localStorage.getItem(environment.hashMailSgdea));

    // Guardar los datos adicionales en el localStorage
    (this.dataHashMailSgdea.mailBox = this.mailBox),
      (this.dataHashMailSgdea.dataEmail = this.eventClickButtonSelectedData);
    let hashMailSgdea = this.authService.encryptAES(this.dataHashMailSgdea);
    localStorage.setItem(environment.hashMailSgdea, hashMailSgdea);

    // Procesa el parametro y enviarlo por get a la creacion de radicado
    let param = btoa(hashMailSgdea);
    param = param.replace(/_/g, "/");
    param = param.replace(/-/g, "+");

    this.validateselectedData(this.dataHashMailSgdea.dataEmail).then(
      (resp) => {

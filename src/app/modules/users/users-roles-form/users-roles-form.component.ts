...
export class UsersRolesFormComponent implements OnInit, OnDestroy {
  ...
  /** BreadcrumbOn  */
  @Input() breadcrumbOn = [{ name: "Gestión de usuarios", route: "/users" }];
  @Input() breadcrumbRouteActive = "Administrar perfiles";
  versionApi = environment.versionApiDefault;
  sgdaVal = environment.sgda.ocultarModulos;
  ...
  filteredlistNivelBusq: ReplaySubject<ListaBusq[]> = new ReplaySubject<
    ListaBusq[]
  >(1);
  ...
  ngOnInit() {
    ...
    this.rolesForm = this.formBuilder.group({
      ...
      idRolNivelBusqueda: new UntypedFormControl(
        "",
        Validators.compose([Validators.required]),
      ),
       ...
       */
      listNivelBusqFilter: new UntypedFormControl(
        "",
        Validators.compose([
          // Validators.required
        ]),
    ...
    if (this.rolesForm.valid) {
      if (this.dataOperationsTrue.length === 0) {
        this.sweetAlertService.sweetInfo(
          this.msgSelectMinPermiso[this.activeLang],
          "",
        );
      } else {
  ...
  getRolOperacionesAll(val) {
    ...
    this.restService
      .restGetParams(
        this.versionApi + "roles/roles-operaciones/index-all",
        params,
        val,
      )
      .subscribe(
        (res) => {
          ...
          this.globalAppService
            .resolveResponse(
              this.responseServiceOperaciones,
              true,
              "/users/roles-index",
            )
            .then((res) => {
              ...
              if (responseResolveResponse == true) {
                ...
                this.operations =
                  this.responseServiceOperaciones.data.operaciones;
              }
            });
        },
        (err) => {
          ...
          this.globalAppService
            .resolveResponseError(
              this.responseServiceOperacionesErr,
              true,
              "/users/roles-index",
            )
            .then((res) => {});
  ...
  getListNivelBusq() {
    this.restService
      .restGet(
        this.versionApi + "roles/roles/index-list-nivel-busqueda",
        this.authorization,
      )
      .subscribe(
        (data) => {
          ...
          // Evaluar respuesta del servicio
          this.globalAppService
            .resolveResponse(this.resSerlistNivelBusq)
            .then((res) => {
              let responseResolveResponse = res;
              if (responseResolveResponse == true) {
        ...
        (err) => {
          ...
          // Evaluar respuesta de error del servicio
          this.globalAppService
            .resolveResponseError(this.resSerlistNivelBusqErr)
            .then((res) => {});
        },
      );
  }
  ...
  onSearchId(id, authori) {
    ...
    this.restService
      .restGetParams(this.versionApi + "roles/roles/index-one", params, authori)
      .subscribe(
        (res) => {
                ...
                if (this.responseServiceFormSubmit.data) {
                  for (let name in this.responseServiceFormSubmit.data) {
                    if (this.rolesForm.controls[name]) {
                      this.rolesForm.controls[name].setValue(

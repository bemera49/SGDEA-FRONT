...
@Component({
  selector: 'app-filing-web-page-index',
  templateUrl: './filing-web-page-index.component.html',
  styleUrls: ['./filing-web-page-index.component.css']
})
export class FilingWebPageIndexComponent implements OnInit {
  versionApi = environment.versionApiDefault;
  breadcrumbOn = [{ 'name': 'Radicación', 'route': '/filing' },];
  breadcrumbRouteActive = 'Página web';
  ...
  resErrServices: any;
  route: string = '/filing/filing-web-index';
  ...
  configDataTable: any = {
    ...
    actions: [
      { icon: 'visibility', routerLink: '/filing/filing-update-web-page', 
      label: 'Ver' },
    ],
    TableMenuOptions: [
  ...
  public menuReceiveData(event) {
    ...
    switch (event.action) {
      case 'asignar':
        this.router.navigate(['/' + this.initBotonViewRoute + '/' + btoa(this.eventClickButtonSelectedData[0]['id'])]);
      break;
      case 'edit':
        this.router.navigate(['/' + this.initBotonUpdateRoute + '/' + this.eventClickButtonSelectedData[0]['data'][0]+"/web"]);
      break;
      case 'view':

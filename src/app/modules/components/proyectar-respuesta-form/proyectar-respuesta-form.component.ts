...
import { Observable, catchError, finalize, forkJoin, retry, startWith } from "rxjs";
...
import { Decrypted, IMezclaCorrespondenciaResponse, IProyectarRespuestaBody, ResponseAsociarPlantillas, ResponseAsociarPlantillasData, ResponseReasociarPlantilla } from "./interfaces";
...
@Component({
  selector: 'app-proyectar-respuesta-form',
  templateUrl: './proyectar-respuesta-form.component.html',
  styleUrls: ['./proyectar-respuesta-form.component.css'],
  standalone: true,
  imports: [
    CommonModule,
    MaterialModule,
...
export class ProyectarRespuestaFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.formGroupPlantillas.get('plantilla').valueChanges.subscribe(newValue => console.log(newValue))
        ...
        () => {
          ...
          this.postProyectarRespuesta(dataForm).pipe(
              ...
              return this.postAsociarPlantilla(response.decrypted.id, plantilla.id).pipe(
                ...
                switchMap((responseAsociarPlantilla: ResponseAsociarPlantillas) => {
                  ...
                  return this.postMezclaCorrespondencia
                    (responseAsociarPlantilla.data, decrypted).pipe(this.handleError('Mezcla de correspondencia'),
                      switchMap((responseMezclaCorrespondencia: any) => {
                        ...
                        if (this.trabajoColaborativoRegistros.length > 0) {
                          return this.postTrabajoColaborativo().pipe(
                            this.handleError('Trabajo colaborativo'),
                            switchMap((responseTrabajoColaborativo: any) => {

                              const user = this.trabajoColaborativoRegistros[0];

                              console.log(responseTrabajoColaborativo)
                              return forkJoin([
                                this.postCambiarEstado(this.currentID, 'EST-044').pipe(this.handleError('Cambio de estado'), retry(3)),
                                this.postCambiarEstado(this.idRadicadoSalida, 'EST-044').pipe(this.handleError('Cambio de estado'), retry(3)),
            ...
            this.modal.openNotify('Éxito', 'Proyección por respuesta realizada exitosamente', true).afterClosed().subscribe(res => {
              this.router.navigate([`pqrs-view/${this.currentID}/ver-respuesta-proyectada/${this.idRadicadoSalida}`])

...
export class UploadMassiveFilesDetailModalComponent implements OnInit, AfterViewInit {
  ...
  @Input() maxSize: number = 26214400; // Maximo de peso permitido por defecto 5MB

  @Input() maxSizeText: string = '25MB';
  ...
  ngOnInit() {
    ...
    if (this.dataSend.isUpdate != undefined && this.dataSend.isUpdate == true) {
      this.hideSwPublic = true;
    }
                ...
                this.encryptService.decryptAES(this.responseService.encrypted, this.authorization).then((res) => {
                  ...
                  this.globalAppService.resolveResponse(responseServiceDecrypt, false, this.redirectionPath).then((res) => {
                    ...
                    if (responseResolveResponse == true) {
                      ...
                      console.log({ "respuesta modal": responseServiceDecrypt });
                      ...
                      if (responseServiceDecrypt.decrypted != undefined) {
                        let message = this.translate.instant(responseServiceDecrypt.decrypted.message);
                        this.sweetAlertService.showNotification('success', message);
                        setTimeout(() => {
      ...
      this.validateFileExtension(event.target.files[0]).then((res) => {
        if (res) {
          if (event.target.files[0].size > this.maxSize) {
            ...
            msjmaxTamanyo = this.translate.instant('Solo es permitido cargar'); // El tamaño del archivo supera el tamaño permitido <<valor máximo>> MB.
            msjmaxTamanyo = msjmaxTamanyo + ' <strong>' + this.translate.instant('el valor máximo es') + ': </strong>' + this.maxSizeText;
          ...
          this.sweetAlertService.sweetInfoText('Archivo no válido', notificationErrExt + ' tipo documento, imagen, video, audio o comprimidos'); //+ this.notificationErrExtArray.join());
          this.uploadValid = false;
          this.inputFile.nativeElement.value = '';
          this.nombreArchivoSeleccionado = "";
        }

...
export class ObservationLoansDialog implements OnInit, AfterViewChecked {
  ...
  constructor( public dialogRef: MatDialogRef<ObservationLoansDialog>, @Inject(MAT_DIALOG_DATA) public data: DialogData,
    ...
    this.moduleForm = this.formBuilder.group({
      ...
      numeroSap: new UntypedFormControl('', Validators.compose([
        // Validators.required,
      ])),
      numeroFolios: new UntypedFormControl('', Validators.compose([
        // Validators.required,
      ])),
      /**
      ...
      fechaInicioPrestamo: new UntypedFormControl('', Validators.compose([
        // Validators.required,
      ])),
      fechaFinPrestamo: new UntypedFormControl('', Validators.compose([
        // Validators.required,
      ])),
  ...
  submitForm() {
    if (this.moduleForm.valid) {
      ...
      const errors = {}; // Object to store specific error messages

      // Loop through form controls and collect errors
      for (const controlName in this.moduleForm.controls) {
        if (this.moduleForm.get(controlName).invalid) {
          errors[controlName] = this.moduleForm.get(controlName).errors; // Store errors for specific fields
        }
      }

      // Display specific error messages (replace with your desired notification method)
  ...
  onTipoPrestamoChange(event: any): void {

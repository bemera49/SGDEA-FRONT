...
import { IOEvents, SocketHelper } from "@app/services/socket.helper";
...
export class AdminSideNavBarComponent implements OnInit {
  ...
  notificationsList: { id: number; name: string; leido: number }[] = [];
  ...
  constructor(
    ...
    // iniciar escucha de socket por el canal del usuario logeado
    let dataUser = this.authService.decryptAES(
      localStorage.getItem(environment.hashSgdea),
    );
    this.socket.emit(IOEvents.join_room, {
      channel: dataUser.data.idDataCliente,
    });
    this.socket.on(IOEvents.notificador, (data: any) => {
      this.tastVc.open({
        title: data.data,
        description: data.msj,
        duration: 10000,
        success: true,
      });
    });
  ...
  getInitials(): string {
    const name = this.userData.nombre.split(" ");
  ...
  ngOnInit() {
    ...
    this.restService.updateBreadcrumbs([
      {
        link: "/",
        name: "Inicio",
      },
    ...
    this.restService.breadcrumbs$.subscribe((bc) => (this.breadcrumbs = bc));
  }
        ...
        if (typeof this.responseJsonSubMenus[menuArr.ruta] != "undefined") {
          ...
          this.responseJsonSubMenus[menuArr.ruta].forEach((element) => {
            dataService.forEach((elementService) => {
              if (
                element.operacion == elementService ||
                menuArr.ruta + "%" + element.operacion == elementService
              ) {
  ...
  verNotificacionesBtn() {
    this.verNotificaciones = !this.verNotificaciones;
    forkJoin([
      ...(this.notificationsList.map((el) =>
        this.restService.put("api/notificacion/cambiar-estado", el.id, {}),
      ) as Observable<any>[]),
    ]).subscribe({
      next: (res) => {
        this.notificationsList.map((el) => (el.leido = 1));
  ...
  async getNofitications() {
    lastValueFrom(
      this.restService.get(
        `api/usuario/${this.restService.getUserData().idDataCliente}/notificaciones`,
      ),
    ).then((res: any) => {
      this.notificationsList = res.data.filter((el) => el.leido == 0);
    });
  }
}

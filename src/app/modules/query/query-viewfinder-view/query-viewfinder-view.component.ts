...
@Component({
  selector: 'app-query-viewfinder-view',
  templateUrl: './query-viewfinder-view.component.html',
  styleUrls: ['./query-viewfinder-view.component.css']
})
export class QueryViewfinderViewComponent implements OnInit {
  ...
  paramiD: string;
  paramOID: string;
  paramDab: string; // parametro de tabla
  paramDabOk: string;
  ...
  breadcrumbOn = [
    ...
    { 'name': 'Consulta ADI', 'route': this.redirectionPath }
  ];
  ...
  ruoteServiceDownloadFile: string = 'consultas/consulta-adi/download-document';
  ...
  constructor(private route: ActivatedRoute, public lhs: LocalStorageService, public sweetAlertService: SweetAlertService, public restService: RestService, private router: Router,
    ...
    this.paramDab = this.route.snapshot.paramMap.get('dab'); // SE recibe el id
    this.paramDabOk = ConvertParamsBase64Helper(this.paramDab); // Se pasa al html como componete para que reciba el ID
    // this.routeBotonUpdateView = this.routeBotonUpdateView + this.paramiD;

    // valida la ruta a consultar
    switch(this.paramDabOk) {
      case 'Recibido':
        this.reuteLoadView = 'consultas/consulta-adi/view-recibido'; // Ruta a consultar en el ViewList
      break;
      case 'Interno':
        this.reuteLoadView = 'consultas/consulta-adi/view-interno'; // Ruta a consultar en el ViewList
      break;
      case 'Externo':
        this.reuteLoadView = 'consultas/consulta-adi/view-externo'; // Ruta a consultar en el ViewList
      break;
    }
  }

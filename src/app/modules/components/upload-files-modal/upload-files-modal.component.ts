...
export interface DialogData {
  ...
  redirectionPath: any;
  validUploadCorrespondencia?: any
}
...
export class UploadFilesModalComponent implements OnInit {
  ...
  @Input() validUploadCorrespondencia: boolean = false;
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(UploadFilesDialog, {
      ...
      data: {
        ...
        validUploadCorrespondencia: this.validUploadCorrespondencia
      }
    });
...
export class UploadFilesDialog implements OnInit, AfterViewChecked {
  ...
  validUploadCorrespondencia: boolean = false
  ...
  constructor(public dialogRef: MatDialogRef<UploadFilesDialog>, @Inject(MAT_DIALOG_DATA) public data: DialogData, private formBuilder: UntypedFormBuilder,
    ...
    */
    ...
    this.validUploadCorrespondencia = this.data.validUploadCorrespondencia;
  ...
  submitForm() {
    ...
    if (!this.moduleForm.valid) {
      ...
      if (this.validUploadCorrespondencia)
        this.sendUploadGuias(formData);
      else
      ...
      switch (event.type) {
        ...
        case HttpEventType.Response: // Cuando termina la carga
          ...
          if (event.body) {
            ...
            this.uploadResponse = { status: true, message: 'Completado', proccess: 100 };
            let error = event.body.success ? '' : event.body.error;
            this.sweetAlertService.showSwal('title-and-html', 'Carga Guías masivo', event.body.message + "<br>" + error);

...
export class ResetPassMainComponent implements OnInit, OnDestroy {
  ...
  configInicial(): void {
    ...
    this.authService.authPost(`${environment.versionApiDefault}site/cs-inicial`, data, false).subscribe((responseApi: IConfigInicial) => {
  ...
  configParams(): void {
    ...
    this.authService.authPost(`${environment.versionApiDefault}site/cs-params`, data, false).subscribe((responseApi: IConfigParams) => {
      this.globalAppService.resolveResponse(responseApi).then((responseGlobal) => {
        if (responseGlobal == true) {

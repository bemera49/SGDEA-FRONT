...
@Component({
  selector: 'app-settings-app-provides-external-update',
  templateUrl: './settings-app-provides-external-update.component.html',
  styleUrls: ['./settings-app-provides-external-update.component.css']
})
export class SettingsAppProvidesExternalUpdateComponent implements OnInit {
  ...
  textForm = 'Actualizar usuario interoperabilidad'; // i18n
  // Ruta a redirigir
  ...
  submitFormReceive(data) {
    ...
    this.restService.restPut( this.versionApi + 'configuracionApp/cg-proveedores-externos/update', data, this.authorization)
      .subscribe((res) => {

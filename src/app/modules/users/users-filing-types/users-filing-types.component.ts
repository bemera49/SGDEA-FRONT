...
@Component({
  selector: 'app-users-filing-types',
  templateUrl: './users-filing-types.component.html',
  styleUrls: ['./users-filing-types.component.css']
})
export class UsersFilingTypesComponent implements OnInit, OnDestroy {

  // Datos del formulario
  textButtonForm = 'Actualizar'; // Nombre del boton
  textFormRol = 'Modificar tipos de radicado del perfil'; // Nombre del formulario
  ...
  breadcrumbRouteActive = 'Actualizar tipos de radicado';
  ...
  messajeSelectMinPermiso = 'El perfil debe tener al menos un acceso habilitado';
  ...
  ngOnInit() {
    ...
    this.form = this.formBuilder.group({
      ...
      idCgTipoRadicado: new UntypedFormControl('', Validators.compose([
        // Validators.required
      ])),
    });
  ...
  getDocumentaryTypesAll( val ) {
    ...
    this.restService.restGetParams(versionApi + 'configuracionApp/tipos-radicados/index-all', params, val)
      .subscribe((res) => {
  ...
  submit() {
    ...
    this.form.controls['idCgTipoRadicado'].setValue(this.dataOperationsTrue); // Se pasa el array de ids al campo del formulario
    ...
    if (this.form.valid) {
      if (this.dataOperationsTrue.length === 0) {
        let messageError = {
          error: [ this.translate.instant(this.messajeSelectMinPermiso) ]
        }
        this.sweetAlertService.sweetInfo('Algo está mal', messageError);
      } else {
        // console.log( this.form.value );
        this.restService.restPut(versionApi + 'roles/roles/create-rol-tipo-radicado', this.form.value, this.authorization)
          .subscribe((res) => {

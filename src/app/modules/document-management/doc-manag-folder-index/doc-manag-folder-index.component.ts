...
export class DocManagFolderIndexComponent implements OnInit, OnDestroy {
  ...
  routeLoadDataTablesService: string =
    environment.versionApiDefault + "gestionDocumental/expedientes/index";
  // Configuración para el proceso change status
  routeChangeStatus: string =
    environment.versionApiDefault +
    "gestionDocumental/expedientes/change-status";
  ...
  dtTitles: any = [
    ...
    { title: "Codigo del expediente", data: "numeroExpediente" },
    { title: "Nombre del expediente", data: "nombreExpediente" },
    ...
    { title: "Otro", data: "otro" },
    { title: "Numero de documento", data: "numeroDocumentos" },
    { title: "Numero de folios", data: "numeroFolios" },
    { title: "Soporte", data: "soporte" },
    { title: "Frecuencia de consulta", data: "frecuenciaConsulta" },
  ...
  menuButtonsSelectNull: any = [
    { icon: "add", title: "Nuevo expediente", action: "add", data: "" },
  ];
  menuButtonsSelectOne: any = [
    { icon: "add", title: "Nuevo expediente", action: "add", data: "" },
    //{ icon: 'edit', title: 'Editar', action: 'edit', data: '' },
    { icon: "remove_red_eye", title: "Ver expediente", action: "view", data: "" },
    // { icon: 'attachment', title: 'Carga de documentos', action: 'attachment', data: '' },
    /*     {
          icon: "cloud_download",
          title: "Descargar paquete de documentos",
          action: "downloadExpedientPackage",
          data: "",
        }, */
  ...
  ruoteServiceDocumentsAne: string =
    environment.apiUrl +
    this.versionApi +
    "radicacion/transacciones/upload-document-to-expedient";
  ...
  statusModalSoliciteLoan: boolean = false; // Muestra u oculta el modal de referencia cruzada

  ruoteServiceDownloadFile: string =
    "gestionDocumental/expedientes/download-electronic-index";
  ...
  constructor(
    ...
    private sendDataService: ProrrogaService
  ) {
    /** Idioma inical */
  ...
  menuReceiveData(event) {
    ...
    switch (event.action) {
      ...
      case "attachment":
        ...
        this.dataDepen = {
          idGdTrdDependencia:
            this.eventClickButtonSelectedData[0].idGdTrdDependencia,
  ...
  processPeaceAndSafe() {
    ...
    this.restService
      .restPost(
        `${this.versionApi}${this.routeServicePeaceAndSafe}`,
        data,
        this.authorization
      )
      .subscribe(
        (responseApi) => {
          this.globalAppService
            .resolveResponse(responseApi, false)
            .then((responseGlobal) => {
  ...
  openModalLoanSolicite() {
    this.router.navigate(["/documentManagement/folder-loan", 'ViewDetail']);
    const data = {
      title: "CREAR SOLICITUD DE PRESTAMO DOCUMENTAL",
      data: this.eventClickButtonSelectedData,
    };
    this.sendDataService.senData(data);
  }
    ...
    if (event.length > 0) {
      ...
      if (event.length == 1) {
        ...
        if (event[0].activarPazYsalvo) {
          const findPeaceAndSafe = this.menuButtons.find(
            (element) => element.action === "peaceAndSafe"
          );
          if (!findPeaceAndSafe) {
          ...
          if (
            ...
            this.validaButtons(event, "closeFolder").then((resTipo) => {
              if (!resTipo) {
                ...
                this.menuButtons = [...this.menuButtons, this.menuButtonsDoc]
                /* this.menuButtons.push(
                  this.menuButtonsDoc,
                  this.menuButtonsClose
                ); */
              }
            });
            ...
            this.validaButtons(event, "crossReference").then((resTipo) => {
              if (!resTipo) {
                ...
                this.havePermissionCrossReference(event).then(
                  (resPermission) => {
                    if (resPermission) {

                      /* this.menuButtons = [...this.menuButtons,this.menuButtonsCrossReference]; */
                      // Agrega la transacción
                      /* this.menuButtons.push(
                        this.menuButtonsCrossReference,
                      ); */
                    }
                  }
                );
            ...
            this.validaButtons(event, "openFolder").then((resTipo) => {
              if (!resTipo) {
                ...
                /*                 this.eliminarRegistro("crossReference");
                                this.eliminarRegistro("soliciteLoan"); */

                console.log('menu boton opciones', this.menuButtons);
                // Agrega la transacción
                this.menuButtons = [...this.menuButtons, this.menuButtonsOpen]
              }
            });
          }
        } else {
  ...
  dowloadDocuments() {
    ...
    this.restService
      .restPut(
        this.versionApi + this.ruoteServiceDownloadFile,
        data,
        this.authorization
      )
      .subscribe(
        (res) => {
          ...
          this.globalAppService
            .resolveResponse(this.resServicesDownload, false)
            .then((res) => {
              let responseResolveResponse = res;
              if (responseResolveResponse == true) {
                this.downloadFile(
                  this.resServicesDownload.datafile,
                  this.resServicesDownload.fileName
                );
                this.sweetAlertService.showNotification(
                  "success",
                  this.resServicesDownload["message"]
                );
              }
              // Cargando false
              this.sweetAlertService.sweetClose();
        ...
        (err) => {
          ...
          this.globalAppService
            .resolveResponseError(this.resServicesDownloadErr)
            .then((res) => { });
        }
      );
  }
  ...
  transacctionCloseFolder(data) {
    ...
    this.restService
      .restPost(
        this.versionApi + "gestionDocumental/expedientes/closed-expedient",
        params,
        this.authorization
      )
      .subscribe(
        (res) => {
          ...
          this.globalAppService
            .resolveResponse(this.resSerCloseFolder, false)
            .then((res) => {
              let responseResolveResponse = res;
              if (responseResolveResponse == true) {
                this.sweetAlertService.showNotification(
                  "success",
                  this.resSerCloseFolder.message
                );
                if (this.resSerCloseFolder.datafile) {
                  this.downloadFile(
                    this.resSerCloseFolder.datafile,
                    this.resSerCloseFolder.fileName
                  );
                }
                this.changeChildrenService.changeProcess({
                  proccess: "reload",
                });
              }
              // Cargando false
        ...
        (err) => {
          ...
          // Evaluar respuesta de error del servicio
          this.globalAppService
            .resolveResponseError(this.resSerCloseFolderErr, false)
            .then((res) => { });
        }
      );
  }
  ...
  transacctionOpenFolder(data) {
    ...
    this.restService
      .restPost(
        this.versionApi + "gestionDocumental/expedientes/open-expedient",
        params,
        this.authorization
      )
      .subscribe(
    ...
    swal({
      ...
      if (result.value) {
        ...
        this.restService
          .restPost(
            this.versionApi +
            "gestionDocumental/expedientes/download-file-documents",
            data,
            this.authorization
          )
          .subscribe(
            (res) => {
              ...
              this.globalAppService
                .resolveResponse(
                  this.resServicesDownloadExpedientPackage,
                  false
                )
                .then((res) => {
                  let responseResolveResponse = res;
            ...
            (err) => {
              ...
              this.globalAppService
                .resolveResponseError(
                  this.resServicesDownloadExpedientPackageErr,
                  false
                )
                .then((res) => { });
            }
  ...
  validateFileWtatus() {
    ...
    this.restService
      .restPost(
        this.versionApi + "gestionDocumental/expedientes/notification-status",
        params,
        this.authorization
      )
      .subscribe(

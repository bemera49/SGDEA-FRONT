...
/** Las variables para mostrar la alerta informativa  */
  ...
  constructor( public dialogRef: MatDialogRef<ArchiveDialog>, @Inject(MAT_DIALOG_DATA) public data: DialogData,
      ...
      public globalAppService: GlobalAppService, public lhs: LocalStorageService, private modal: ModalService ) {
    /**
  ...
  ngOnInit() {
    ...
    // listen for search field value changes
    this.moduleForm.controls['listPasilloFilter'].valueChanges
    .pipe(takeUntil(this._onDestroy))
    .subscribe(() => {
      this.filterBanks('listPasillo');
    });
    // listen for search field value changes
    this.moduleForm.controls['listModuloFilter'].valueChanges
    .pipe(takeUntil(this._onDestroy))
    .subscribe(() => {
      this.filterBanks('listModulo');
    });
    ...
    this.moduleForm.controls['listCajasFilter'].valueChanges
    .pipe(takeUntil(this._onDestroy))
    .subscribe(() => {
      this.filterBanks('listCajas');
    });

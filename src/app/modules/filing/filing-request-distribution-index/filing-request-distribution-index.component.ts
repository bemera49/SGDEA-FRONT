...
@Component({
  selector: 'app-filing-request-distribution-index',
  templateUrl: './filing-request-distribution-index.component.html',
  styleUrls: ['./filing-request-distribution-index.component.css']
})
export class FilingRequestDistributionIndexComponent implements OnInit {
  versionApi = environment.versionApiDefault;
  breadcrumbOn = [{ 'name': 'Radicación', 'route': '/filing' },];
  breadcrumbRouteActive = 'Distribución de solicitudes';
  ...
  resErrServices: any;
  route: string = '/filing/sdqs-requests-index';
  ...
  configDataTable: any = {
    ...
    actions: [
      { icon: 'visibility', routerLink: '/filing/filing-update-request', 
      label: 'Ver' },
    ],
    TableMenuOptions: [
  ...
  public menuReceiveData(event) {
    ...
    switch (event.action) {
      ...
      break;
      case 'edit':
        this.router.navigate(['/' + this.initBotonUpdateRoute + '/' + this.eventClickButtonSelectedData[0]['data'][0]+"/sdqs"]);
      break;
      case 'view':

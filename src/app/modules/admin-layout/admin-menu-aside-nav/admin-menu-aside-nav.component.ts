...
@Component({
  selector: 'app-admin-menu-aside-nav',
  templateUrl: './admin-menu-aside-nav.component.html',
  styleUrls: ['./admin-menu-aside-nav.component.css']
})
export class AdminMenuAsideNavComponent implements OnInit {
  ...
  toggleSubMenu(index: number): void {
    if (this.openSubMenuIndex === index) {
      ...
      this.activeItem = this.menuItems[index].id;
    }
  }

  selectSubItem(itemId: string): void {
    this.toggleMenuVisibility();
  }

  isItemActive(menuitem: any): boolean {
    const currentRoute = this.router.url.split('/').slice(0, 3).join('/');
    return currentRoute.includes(menuitem.ruta);
  }

...
export class FilterInitialListDialog implements OnInit {
  ...
  submit() {
    ...
    if (this.form.valid) {
      if (this.model.filterOperation[0].fechaInicial && this.model.filterOperation[0].fechaFinal) {

        const fechaInicial = new Date(this.model.filterOperation[0].fechaInicial);
        const fechaFinal = new Date(this.model.filterOperation[0].fechaFinal);
        if (fechaInicial > fechaFinal) {
          this.toastService.open({ title: 'Error', description: 'La fecha inicial no puede ser mayor que la fecha final.', duration: 2000, success: false });
          return;
        }
        const today = new Date();
        if (fechaFinal > today) {
          this.toastService.open({ title: 'Error', description: 'La fecha final no puede ser mayor que la fecha actual.', duration: 2000, success: false });
          return;
        }
      }

...
export class SettingsAppFilingTypesFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.filingTypesForm.controls['listEstructuraFilter'].valueChanges
    .pipe(takeUntil(this._onDestroy))
    .subscribe(() => {
      this.filterBanks('estructura');
    });
  ...
  getTokenLS() {
    ...
    this.lhs.getToken().then((res: string) => {
      ...
      this.getEstructuras(this.authorization);
      if (this.paramOID != 0) {
          ...
          this.globalAppService
            ...
            .then((res) => {
              ...
              if (responseResolveResponse == true) {
                ...
                this.validaCapoMultiple();
                let textStructura;
                this.filteredlistEstructura.subscribe(list => {
                  try {
                    textStructura = list.find(item => item.id === this.responseServiceFormSubmit.data["estructura"]);
                    this.verPrevia(textStructura.val);
                  } catch (error) {
                    console.error("Error buscando estructura:", error);
                  }
                });
                // Cargando false
  ...
  getEstructuras(auth){
    let versionApi = environment.versionApiDefault;
    this.restService.restGet( versionApi + 'configuracionApp/numero-radicado/index-estructuras', auth).subscribe(
      (res) => {
  ...
  filterBanks(nomList) {
    ...
    let search = this.filingTypesForm.controls[nomList + 'Filter'].value;
    if (!search) {
  ...
  verPrevia(event) {
    ...
    const codigoCgTipoRadicado = this.filingTypesForm.get('codigoCgTipoRadicado').value;
    const nombreCgTipoRadicado = this.filingTypesForm.get('nombreCgTipoRadicado').value;
    const firstLetterNombre = nombreCgTipoRadicado.charAt(0);

    const formattedString = selectedText
      .replace(/yyyy/g, currentYear)
      .replace(/mm/g, currentMonth)
      .replace(/dd/g, currentDay)
      .replace(/depe/g, '10000')
      .replace(/consecutivo/g, '1')
      .replace(/tiporad/g, codigoCgTipoRadicado)
      .replace(/letratiporad/g, firstLetterNombre)
      .replace(/regional/g, 'bogotá');

    console.log('Selected text:', selectedText);
    console.log('Formatted string:', formattedString);
    this.previa = ' '+formattedString;
  }

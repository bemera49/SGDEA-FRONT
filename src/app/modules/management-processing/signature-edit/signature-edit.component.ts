...
export class SignatureEditComponent implements OnInit {
  ...
  nextStep() {
    ...
    let updatedBody = body$.map((item) => {
      let userProperty =
        item.tipo_usuario === "Interno"
          ? { user_id: item.user_id }
          : { user_externos_id: item.user_externos_id };
      ...
      return {
        ...
        coordenada: '{"x": 0, "y": 0}',
        fecha_maxima_firma: this.parseDateToPut(
    ...
    this.modal.openAction("¿Está seguro de la acción a realizar?", "", () => {
      this.signature$
        .editSignature(this.currentID, updatedBody)
        .pipe(
          catchError((err) => {
            this.toast.open({
        ...
        .subscribe((res: any) => {
          this.modal
            .openNotify("Éxito", "Guardado de información exitosoo", true)
            .afterClosed()
            .subscribe((res) => this.router.navigate(["/signin"]));
        });
    });
  ...
  ngOnInit(): void {
    this.rest$
      .get(`api/flujo/radicado/lista/mejorado?idRadicado=${this.currentID}`)
      .pipe()
      .subscribe((res) => {
        let maximunDate = res.flujo_firma[0].fecha_maxima_firma;
        this.formFecha
          .get("fechaLimite")
          .setValue(this.parseDateFromGet(maximunDate));
        ...
        let body = res.flujo_firma.map((item) => {
          let userProperty =
            item.tipo_usuario === "Interno"
              ...
              : { user_externos_id: item.id };
          let secondUserProperty =
            item.tipo_usuario === "Interno"
              ? { id: item.usuario.id }
              : { id: item.id };
          let nameProperty =
            item.tipo_usuario === "Interno"
              ? { name: item.usuario.user_detalle.full_name }

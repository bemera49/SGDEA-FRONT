...
export class GlobalAppService {
  ...
  subMenuGet() {
    ...
    this.routeAssetsGlobalMenu = `assets/globals/sub-menu-${language}.json?cacheBust=${new Date().getTime()}`;

    return new Promise(resolve => {
      this.http.get(this.routeAssetsGlobalMenu).subscribe(data => {
        resolve(data);
        // console.log(data);
      }, err => {
        // console.log(err);
  ...
  logout() {
    localStorage.removeItem(environment.hashSgdea);
  ...
  text18nGet() {
    ...
    return new Promise(resolve => {
      this.http.get(this.textLanguage + '.json', { headers: { 'Cache-Control': 'no-cache' } }).subscribe(data => {
        resolve(data);
      }, err => {
        console.error(err);
      });

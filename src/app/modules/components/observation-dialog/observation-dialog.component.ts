...
export class ObservationComponent implements OnInit {
  ...
  @Input() isRejected: boolean; // Id Estado Radicado
  @Input() residingId: number;
...
export class ObservationDialog implements OnInit, AfterViewChecked {
  ...
  constructor( public dialogRef: MatDialogRef<ObservationDialog>, @Inject(MAT_DIALOG_DATA) public data: DialogData,
   ...
   this.isRejected = this.data.isRejected;
   this.residingId = this.data.residingId;
  ...
  ngOnInit() {
    ...
    if (this.isRejected) {
      this.getCasualRechazo();
    }


    // Modulo de reasignacion
    if (this.showReasignacion || this.isRejected) {
  ...
  getCasualRechazo(){
  this.restService.get(this.versionApi + 'api/asignacion/causal-devolucion/' + this.residingId).subscribe( //  + this.residingId
    (data) => {
      this.dataRejected = data;
      console.log({"causal-rechazo": data})
      // Evaluar respuesta del servicio

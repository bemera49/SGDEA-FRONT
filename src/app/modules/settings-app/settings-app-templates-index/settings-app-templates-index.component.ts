...
export class SettingsAppTemplatesIndexComponent implements OnInit {
  ...
  constructor(private toastSvc:ToastService, private templateSettingsSvc:TemplateSettingsService, private router: Router, private floatingButtonService: FloatingButtonService, public restService: RestService, public globalAppService: GlobalAppService,
  ...
  menuReceiveData(event) {
    switch (event.action) {
      ...
      break;
      case 'changeStatus':
        this.changeTemplateStatus(this.eventClickButtonSelectedData[0].id)
  ...
  changeTemplateStatus(id:any){
    this.templateSettingsSvc.changeTemplateStatus(id)
          .subscribe({
            error:(err:any) => {
              console.log(err)
              this.toastSvc.open({

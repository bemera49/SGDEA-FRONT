...
export class ValidacionesDGCPComponent implements OnInit {
  ...
  form: FormGroup = this.fb.group({
    justification: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(500)]], // Acepta cualquier string
  });
  form2: FormGroup = this.fb.group({
    observation: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(500)]], // Acepta cualquier string
  });
  ...
  checkAllCheckedItems() {
    if ( this.conditions.every(item => item.isValid !== null ) ) {
        this.onConfirm();
         ...
         this.toastService.open({ title: `Por favor verifique, datos obligatorios incompletos ` , duration: 5000, success: false })
        }
  };
  ...
  onConfirm(){
    ...
    this.dgcpValidationSvc.validateRequestCriteria(fd)
        .subscribe({
          error:(err:any) => {
            if (err.status == 422) {
              this.toastService.open({ title: `Por favor verifique, datos obligatorios incompletos ` , duration: 5000, success: false })
            } else {
              this.toastService.open({ title: `Validación fallída, ${err.error.message} ` , duration: 2000, success: false })
            }
          },
          next:(resp:any) => {
            console.log(resp);
            if (this.allComplies) {
              this.toastService.open({ title: 'Validación exitosa', duration: 2000, success: true });
              this.date = resp.data.fecha_hrs_check;
            } else {
              this.toastService.open({ title: resp.data.alert.text, duration: 2000, success: true });
  ...
  requestExtension(){
    if (this.form.invalid) {
      ...
      const fd = new HttpParams()
        .append('request_id',  this.requestId)
        .append('justification_add_time',  this.form.controls["justification"].value)

      this.dgcpValidationSvc.requestExtension(fd)
            .subscribe({
              error:(err:any) => {
                this.toastService.open({ title: 'Validación fallida ', duration: 2000, success: true })
              },
              next:(resp:any) => {
                this.toastService.open({ title: 'Solicitud exitosa', duration: 2000, success: true })
                this.form.disabled;
              }
            });
    }

...
import { CooperativeModalComponent } from '../../components/cooperative-modal/cooperative-modal.component';
...
export class FilingViewComponent implements OnInit, OnDestroy {
  ...
  statusModalCooperative: boolean = false;
  // Version api
  ...
  routeServiceUploadSignedDocument = `${environment.apiUrl + this.versionApi
  ...
  paramsDownloadCard: any; // Esta variable toma el valor del CARD del documento seleccionado y lo pasa opción de descaga
  ...
  ruoteServiceDownloadRadiEnvio: string = "radicacion/documentos/download-document-cor";
  ruoteServiceDownload: string = this.ruoteServiceDownloadAnexo;
  ...
  operacionesTiposRadicados() {
    ...
    params['mostrarBotonActualizar'] = true;
    this.restService
  ...
  public menuReceiveData(event) {
    ...
    switch (event.action) {
      ...
      case "add":
        ...
        break;
      case "CancelRadicado":
        if (this.dataUserTramitador.medioRecepcion === 'Correos electrónicos') {
          this.router.navigate(["/" + "/filing/filing-radicados-email-index"]);
        } else if (this.dataUserTramitador.medioRecepcion === 'SDQS') {
          this.router.navigate(["/" + "/filing/sdqs-requests-index"]);
        } else if (this.dataUserTramitador.medioRecepcion === 'Portal Web') {
      ...
      case "Ver":
        ...
        this.ruoteServiceDownload = this.paramsDownloadCard === 'cardCorrespondence' ?  this.ruoteServiceDownloadRadiEnvio :  this.ruoteServiceDownloadAnexo
        this.statusModalviewPdf = true;
        break;

      case "Descargar":
        this.dowloadDocuments(this.paramsDownload, this.paramsDownloadCard);
        break;
  ...
  /**
   * Funcion para enviar el correo del radicado
    */
  private correoRecibido: any;
  getCorreoEmitido(event) {
    console.log(event)
    this.correoRecibido = event;
  }

  transactionEnviarCorreoRadicado() {
    this.sweetAlertService.sweetLoading();

    if (this.correoRecibido === 'No aplica') {
      this.sweetAlertService.showNotification("error", "El radicado no tiene correo asignado");
      this.sweetAlertService.sweetClose();
      return;
    }

    console.log("INICIO")
    console.log(this.correoRecibido);
    console.log("FINAL")
    let params = {
      id: this.paramOID,
      correoRemitente: this.correoRecibido,
    };

        // Enviar datos al backend
        this.restService.restPost(
          ...
          (res) => {
            ...
            this.globalAppService.resolveResponse(this.resSerPrintStickers, false).then((responseResolved) => {
              if (responseResolved === true) {
          ...
          (err) => {
            ...
            this.globalAppService.resolveResponseError(this.resSerPrintStickersErr).then(() => {
  ...
  transactionEnviarCorreoRadicado1() {
    this.sweetAlertService.sweetLoading();
    if (this.correoRecibido == 'No aplica') {
      this.sweetAlertService.showNotification("error", "El radicado no tiene correo asignado");
      this.sweetAlertService.sweetClose();
    ...
    params['correoRemitente'] = this.correoRecibido;
    console.log(params);
    this.restService
  ...
  //hace otra fucion para generar un pdf con los radicados seleccionados
  printRadicado() {
    window.print();
  }
  ...
  actionsDocsButton(params) {
    ...
    this.paramsDownloadCard = params.card;
  ...
  dowloadDocuments(idDocuments, card) {
    let data = {
      ButtonSelectedData: {
        0: {
          id: idDocuments,
    ...
    this.sweetAlertService.sweetLoading();
    let url = card === 'cardCorrespondence' ?  this.ruoteServiceDownloadRadiEnvio :  this.ruoteServiceDownloadAnexo
    this.restService.restPut(this.versionApi + url, data, this.authorization).subscribe(
      (res) => {
  ...
  uploadDocument(event) {
    ...
    let titleShow = "Actualizar documento";
    ...
    this.globalAppService.text18nGet().then((res) => {
      ...
      swal({
        ...
        if (result.value) {
          ...
          this.sweetAlertService.showNotification("success", this.resSerLenguage["message"]);
          // Transacion Devolver
...
*/
  ...
  openCooperativeModal(): void {
    const dialogRef = this.dialog.open(CooperativeModalComponent, {
      width: '90%', // Changed to full width
      data: { iconMenu: this.iconMenu, dataSend: this.dataSend }
    });

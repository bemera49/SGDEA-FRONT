...
export class SweetAlertService {
  ...
  showSwal(type, titleMessage, message, html?: any) {
    ...
    this.text18nGet().then((res) => {
      ...
      } else if (type == "title-and-html") {
        Swal({
          type: "info",
          title: titleTranslate,
          width: "auto",
          html: message, // Render HTML content
          buttonsStyling: false,
          confirmButtonClass: "btn btn-info",
        }).catch(Swal.noop);
      }
  ...
  alertaReemplazoExistente(code: string, message: string) {
    Swal({
      title: code,
      text: message,
  ...
  alertIncluirDocument(message: string) {
    Swal({
      text: message,
      showCancelButton: false,
      showConfirmButton: true,
  ...
  text18nGet() {
    ...
    return new Promise((resolve) => {
      this.http.get(this.textLanguage + ".json").subscribe(
        (data) => {
          resolve(data);
          // console.log(data);
        },
